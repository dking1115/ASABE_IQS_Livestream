...
class DataModel(QObject):
    ...
    events_active_changed=pyqtSignal()
    def __init__(self,management=False,name="none"):
        ...
        self.uwb_sub=self.node.create_subscription(PoseStamped,"UWB_position",self.uwb_callback,10)
        self.man_img_sub=self.node.create_subscription(Image,"manuverability_top_down",self.img_callback,10)
        self.dur_odom_sub=self.node.create_subscription(Odometry, "load_toad_odom",self.load_toad_odom_callback,10)
        self.camera_seting_update_pub=self.node.create_publisher(Bool,"camera_settings_updated",10)
        if self.management:
            self.pull_state_pub=self.node.create_publisher(Int8,"pull_state",10)
            self.man_state_pub=self.node.create_publisher(Int8,"man_state",10)
            self.dur_state_pub=self.node.create_publisher(Int8,"dur_state",10)
            self.generate_pull_order(1)
        self.pull_state_sub=self.node.create_subscription(Int8,"pull_state",self.pull_state_callback,10)
        self.man_state_sub=self.node.create_subscription(Int8,"man_state",self.man_state_callback,10)
        self.dur_state_sub=self.node.create_subscription(Int8,"dur_state",self.dur_state_callback,10)
        self.track_extent_obj={"x_min":-10,"x_max":10,"y_min":-10,"y_max":10}
        self.track=[]*19
        self.get_track()
        self.thread()
        self.events_active={"pulls":False,"man":False,"dur":False}
        self.pull_state=0
        self.dur_state=0
        self.man_state=0
    ...
    def change_screen(self,id):
        ...
        self.node.get_logger().info(f"Changing to screen {id}")
    ...
    @pyqtProperty(QVariant,notify=events_active_changed)
    def active_events_qt(self):
        return self.events_active


    def update_active_events(self):
        if self.pull_state==2:
            self.events_active["pulls"]=True
        else:
            self.events_active["pulls"]=False
            if self.pull_state==4:
                if self.man_state==2:
                    self.events_active["man"]=True
                else:
                    self.events_active["man"]=False
                if self.dur_state==2:
                    self.events_active["dur"]=True
                else:
                    self.events_active["dur"]=False
        self.events_active_changed.emit()
        print(self.events_active)
